<template>
  <div class="min-h-screen theme-bg-main pb-24 animate-in fade-in duration-1000">
        ...
        <div class="space-y-4">
          ...
          <div v-if="!loading">
            <h1 class="text-6xl font-black theme-text-main tracking-tighter leading-none mb-4">
              Edit <span class="bg-gradient-to-r from-indigo-500 to-violet-500 bg-clip-text text-transparent">Instructor</span>
            </h1>
            <p class="text-lg theme-text-muted font-medium max-w-xl">Updating <span class="theme-text-main font-black">#{{ instructorId }} {{ form.name }}</span>'s credentials and professional presence.</p>
          </div>
          <div v-else class="space-y-4">
            <div class="h-16 w-80 theme-bg-element rounded-3xl animate-pulse"></div>
            <div class="h-6 w-96 theme-bg-element rounded-xl animate-pulse"></div>
          </div>
        </div>

        <!-- Progress Indicator (Dynamic based on loading) -->
    ...
    <div class="max-w-7xl mx-auto px-8 relative z-10">
      <div v-if="loading" class="grid grid-cols-12 gap-10">
        <div class="col-span-12 lg:col-span-3 space-y-4">
          <div v-for="i in 3" :key="i" class="h-20 theme-bg-card border theme-border rounded-[2rem] animate-pulse"></div>
        </div>
      ...
      <div v-else class="grid grid-cols-12 gap-10">
        ...
        <div class="col-span-12 lg:col-span-3">
          <div class="sticky top-8 space-y-4">
            ...
            <div class="mt-8 p-8 theme-bg-card border theme-border rounded-[2.5rem] shadow-xl relative overflow-hidden group">
               ...
               <h4 class="text-[10px] font-black theme-text-dim uppercase tracking-widest mb-6 border-b theme-border pb-4">Performance Profile</h4>
               <div class="grid grid-cols-2 gap-4 relative z-10">
                 <div class="space-y-1">
                   <p class="text-[10px] font-black uppercase tracking-widest theme-text-dim">Courses</p>
                   <p class="text-2xl font-black tabular-nums theme-text-main">{{ stats.total_courses || 0 }}</p>
                 </div>
                 <div class="space-y-1 text-right">
                   <p class="text-[10px] font-black uppercase tracking-widest theme-text-dim">Students</p>
                   <p class="text-2xl font-black tabular-nums theme-text-main">{{ stats.total_students || 0 }}</p>
                 </div>
                 <div class="space-y-1 col-span-2 pt-4 mt-2 border-t theme-border flex items-center justify-between">
                   <p class="text-[10px] font-black uppercase tracking-widest theme-text-dim">Avg Rating</p>
                   <div class="flex items-center gap-1">
                     <svg class="w-4 h-4 text-amber-500" fill="currentColor" viewBox="0 0 20 20"><path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"/></svg>
                     <p class="text-sm font-black theme-text-main tabular-nums">{{ stats.rating || '0.0' }}</p>
                   </div>
                 </div>
               </div>
            </div>
        ...
        <div class="col-span-12 lg:col-span-9 space-y-10">
          <form @submit.prevent="updateInstructor" class="space-y-10">
              ...
              <div class="theme-bg-card border theme-border rounded-[3rem] p-12 shadow-2xl relative overflow-hidden">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-10 items-start">
                  ...
                  <div class="space-y-8">
                    ...
                    <div class="p-6 theme-bg-element rounded-[2rem] border-2 border-indigo-500/10">
                       <p class="text-[10px] font-black theme-text-main uppercase tracking-widest mb-2">Security Note</p>
                       <p class="text-[10px] font-bold theme-text-dim leading-relaxed italic">Login password remains unchanged. To reset, use the administrative security tools.</p>
                    </div>
                  </div>
            ...
            <!-- Global Action Bar -->
            <div class="mt-12 flex items-center justify-between p-6 theme-bg-card border theme-border rounded-[2.5rem] shadow-2xl backdrop-blur-xl">
              <div class="flex items-center gap-6">
                <div class="hidden md:flex items-center gap-2 px-4 py-2 theme-bg-element rounded-full border theme-border">
                  ...
                  <span class="text-[10px] font-black theme-text-main uppercase tracking-widest">Profile Loaded</span>
                </div>
                <div class="text-[10px] theme-text-dim font-bold uppercase tracking-widest">Phase {{ currentTabIndex + 1 }} of {{ tabs.length }}</div>
              </div>
...
const instructorId = ref(route.params.id);
const stats = ref({});
...
const accountStatusOptions = [
  ...
  { label: 'Profile Rejected', value: 'rejected' }
];
...
const fetchInstructor = async () => {
  loading.value = true;
  try {
    const data = await instructorService.show(instructorId.value);
    const instr = data.data;
    
    Object.assign(form, {
      name: instr.name,
      email: instr.email,
      phone: instr.phone || '',
      avatar: instr.avatar_url,
      headline: instr.headline || '',
      short_bio: instr.short_bio || '',
      bio: instr.bio || '',
      expertise: instr.expertise || '',
      website: instr.website || '',
      linkedin: instr.linkedin || '',
      facebook: instr.facebook || '',
      twitter: instr.twitter || '',
      youtube: instr.youtube || '',
      github: instr.github || '',
      commission_rate: Math.round(instr.commission_rate) || 70,
      status: instr.status || 'pending',
      is_featured: !!instr.is_featured,
      certifications: Array.isArray(instr.certifications) ? instr.certifications : [],
      achievements: Array.isArray(instr.achievements) ? instr.achievements : []
    });

    stats.value = {
        total_courses: instr.total_courses,
        total_students: instr.total_students,
        total_reviews: instr.total_reviews,
        total_earnings: instr.total_earnings,
        rating: instr.rating
    };
  } catch (error) {
...
const addCertification = () => {
  if (!form.certifications) form.certifications = [];
...
const updateInstructor = async () => {
  ...
  errors.value = {};
  try {
    await instructorService.update(instructorId.value, form);
    toast.success('Instructor profile synchronized successfully');
    router.push({ name: 'instructors' });
  } catch (error) {
    if (error.response?.status === 422) {
      errors.value = error.response.data.errors;
      toast.error('Validation failed. Review the highlighted fields.');
      
      if (errors.value.name || errors.value.email || errors.value.phone) activeTab.value = 'general';
      else if (errors.value.headline || errors.value.bio || errors.value.expertise) activeTab.value = 'professional';
    } else {
      toast.error('System error occurred during synchronization');
    }
  } finally {
    saving.value = false;
  }
